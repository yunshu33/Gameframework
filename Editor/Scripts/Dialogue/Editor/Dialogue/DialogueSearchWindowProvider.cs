...
public class DialogueSearchWindowProvider : ScriptableObject, ISearchWindowProvider
{
    ...
    public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
    {
        ...
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            foreach (var type in GetTypesOrNothing(assembly))
            {
                ...
                {
                    var attr = type.GetCustomAttribute(typeof(IsDialogueNodeAttribute), false) as IsDialogueNodeAttribute;
        ...
        foreach (var path in treePaths)
        {
            ...
            tree.Add(new SearchTreeEntry(new GUIContent(path.path.Last())) { level = path.path.Length, userData = path.type });
        }
    ...
    internal void SetConfig(DialogueGraphView dialogueGraphView, DialogueGraphViewEditorWindow dialogueGraphViewEditorWindow)
    {
